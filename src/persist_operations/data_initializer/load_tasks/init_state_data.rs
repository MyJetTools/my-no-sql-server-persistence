...
use crate::{app::logs::Logs, persist_operations::data_initializer::LoadedTableItem};
...
impl InitStateData {
    ...
    pub fn init_table_names(&mut self, tables_names: Vec<String>, logs: &Logs) {
        for table_name in tables_names {
            if let Err(err) = my_no_sql_core::validations::validate_table_name(table_name.as_str())
            {
                logs.add_error(
                    Some(table_name),
